...
</script> <!-- ✅ This closes your main logic script correctly -->
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function startTracking() {
    ...
    fetch("/api/buy", {
      ...
      body: JSON.stringify({ coin: coin, amount: usdAmount })
    })
    .then(res => res.json())
    .then(data => {
      if (data.status === "buy_confirmed") {
        document.getElementById("trackingResults").innerHTML = `
          ✅ Simulated Buy: ${data.amount} USD of ${coin} at $${data.buy_price}<br>
          Tracking live...
        `;

        if (trackingInterval) clearInterval(trackingInterval); // Prevent duplicates

        trackingInterval = setInterval(() => {
          fetch("/api/buy-summary", {
            method: "POST",
            headers: { 'Content-Type': 'application/json' },
          ...
          .then(summary => {
            if (!summary.error) {
              document.getElementById("trackingResults").innerHTML = `
                Coin: <strong>${summary.coin}</strong><br>
                Buy Price: $${summary.buy_price}<br>
                Current Price: $${summary.current_price}<br>
                Coin Amount: ${(summary.initial_investment / summary.buy_price).toFixed(6)}<br>
                Value Now: $${summary.current_value}<br>
                <strong>${summary.gain_loss >= 0 ? 'Gain' : 'Loss'}:</strong> $${summary.gain_loss} (${summary.percent_change}%)
              `;
            }
          });
        }, 5000); // Live tracking every 5s
      }
    });
  }
  ...
  function stopTracking() {
    ...
    document.getElementById("trackingResults").innerHTML += "<br>❌ Tracking Stopped.";
  }

  // ✅ Dynamically add PEPE to dropdown if needed
  const coinSelect = document.getElementById("coinSelect");
  if (coinSelect && ![...coinSelect.options].some(opt => opt.value === "PEPE")) {
  ...
  // ✅ Expose start/stop globally for button use
